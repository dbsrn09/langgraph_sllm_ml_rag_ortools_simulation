...
본 문서는 저장소의 기술 구성, 모듈 간 책임, 데이터·모델·외부 API 의존 관계, 그리고 고도화를 서술합니다. 
<br>다이어그램과 디렉터리 트리는 구조를 한눈에 보여 주고, 
이어지는 설명은 각 구성 요소가 **어떤 입력을 받아 어떤 산출물을 만들며, 어디서 실패할 수 있는지**를 분리하여 기술합니다. 

---

## 배경 및 문서 목적

운영 조직에서의 의사결정은 단일 숫자나 단일 보고서로 완결되지 않습니다.
<br>현황 조회(거래·수주·구매 등), 정책·정의에 대한 근거 확보, 제약 하에서의 자원 배분, 가정 변경에 따른 시나리오 비교, 그리고 사전 경보에 가까운 위험 스코어링이 **동시에** 요구되는 경우가 많습니다. 
<br>본 프로젝트는 이러한 요구를 **하나의 질의 파이프라인**으로 묶되, 각 단계는 **독립 모듈**로 유지하여 변경·검증·감사 추적이 가능하도록 설계되었습니다.

문서 목적은 다음과 같습니다. 
1. 검토자가 코드를 열지 않고도 **기능 경계와 책임 소재**를 파악할 수 있게 할 것. 
2. 재현을 위해 필요한 **환경 변수, 데이터 경로, 스크립트 엔트리**를 한곳에 정리할 것. 
3. LLM·임베딩·최적화 등 **불확실성이 큰 구간**을 명시하여 과도한 신뢰를 방지할 것.

---

## 개요

파이프라인의 중심은 **사용자 질의 한 문장**입니다. 

질의는 먼저 의도 분류(Router)와 실행 계획(Planner)을 거쳐, 필요한 하위 엔진(SQL, RAG, ML, Simulation, OR-Tools)만 순서대로 호출됩니다. 

각 엔진의 결과는 Response 단계에서 **통합 payload**로 묶인 뒤, 자연어 최종 응답으로 정리됩니다. 

이 구조는 “모든 질문에 모든 엔진을 돌리는” 비효율을 줄이고, 장애 시에도 **부분 결과와 오류 맥락**을 남기기 쉽습니다.
...
**“질의 → 구조화된 실행 → 해석 가능한 산출”**의 전 구간을 포괄합니다. 
<br>
각 블록은 개별 스크립트·노트북·서비스 레이어로 분리되어 있어 **단위 검증**이 가능합니다.
...
- Frontend(UI): `front` (React)
- Backend/API: `back` (FastAPI)
- 오케스트레이션: LangGraph + Router + Execution Planner
- sLLM/SQL: Ollama(`text2sql-sllm`) + DuckDB 실행
- RAG: FAISS + OpenAI 임베딩/검색
- ML: Random Forest(로컬 joblib 아티팩트)
- Simulation/OR-Tools: 가정 변경 기반 최적화
- Response/Final Answer: Azure OpenAI 기반 통합 응답 생성
...
아래 다이어그램은 **논리적 실행 순서**를 나타냅니다. 
실제로는 Planner가 반환한 `steps`에 포함된 노드만 실행되며, Response는 실행된 노드의 결과만 취합합니다.
...
**구조 흐름**: 사용자 질의는 Router에서 **어떤 의도(sql/rag/ml/simulation/or_tools 등)가 활성화되는지**를 구조화하고, Planner는 이를 **실행 순서**로 치환합니다. 
<br>LangGraph 쪽 그래프는 해당 순서에 따라 노드를 호출하고, 각 노드는 서비스 레이어(`back/app/langgraph/services/`)를 통해 DuckDB·FAISS·joblib 모델·시뮬레이터·솔버에 도달합니다. 
<br>마지막에 Response 서비스가 Azure OpenAI 채팅 모델을 사용해 **한국어 최종 문장**을 생성합니다.
...
**Ollama**: SQL 경로의 `ask_sllm`은 별도 API 키 없이 로컬 `127.0.0.1:11434`를 사용합니다.
...
다음 이미지는 각 모듈이 **실제로 어떤 형태의 출력**을 내는지와, 최종적으로 UI에서 **통합 응답 카드**로 어떻게 보이는지 시연하기 위한 것입니다. 제출용 자료에 삽입할 때는 민감 정보 가림 처리 여부를 검토합니다.
...
실행 후 `New Chat` 화면에서 질문을 입력하기 전, 추천 질문 칩과 입력 안내가 표시되는 상태입니다. 이후부터는 “통합 응답 카드” 형태로 결과가 순서대로 누적됩니다.

![첫화면 출력](./back/app/image/첫화면출력.png)

#### 8) 최종 통합 응답(권고 요약 + 핵심 요청)

사용자가 질문을 제출하면 최종 응답이 한 카드에 요약됩니다. 카드 내부에 (1) 비율/가정 변경에 따른 전망 요약, (2) 확인이 필요한 항목(예: 학습/요청 대상), (3) 추가 검증 권고/주의 메시지 등 사용자 친화 형태로 정리됩니다.

![통합 응답 출력 1](./back/app/image/출력1.png)

#### 9) 최종 통합 응답(리포트 + 생성 SQL 확인)

통합 응답은 버튼(`View Full Report`, `Save to Report`)으로 상세 리포트를 열 수 있으며, 아래 영역에 `GENERATED SQL`이 함께 표시되어 결과의 근거 쿼리를 직접 확인할 수 있습니다. 또한 업종/영업부문처럼 분석 단위별 집계가 함께 제시됩니다.

![통합 응답 출력 2](./back/app/image/출력2.png)

#### 10) 상세 리포트(테이블 뷰)

리포트 화면에서는 `Report Agent`가 집계 결과를 표 형태로 정리해 보여 줍니다. 테이블에서 항목별 값(예: 연도/분류 단위의 집계 수치)을 확인하고, 필요 시 데이터 에이전트/리포트 에이전트 간 전환으로 추가 맥락을 살펴볼 수 있습니다.

![상세 리포트 테이블 뷰](./back/app/image/출력3.png)
...
- **보안**: 배포 시 인증·TLS·쿼리 제한을 고려해야 합니다.
- **임베딩·채팅 분리**: RAG는 OpenAI 키, 나머지 LLM 단계는 Azure 키를 쓰는 구조라 비용·장애·정책이 이중으로 관리됩니다.
...
본 저장소는 자연어 질의를 출발점으로 하여, **구조화된 실행 계획** 아래에서 SQL·근거 검색·위험 스코어·시뮬레이션·최적화를 선택적으로 수행하고, 마지막에 **통합 자연어 응답**으로 마무리하는 파이프라인을 제시합니다. 
모듈 경계를 명확히 한 덕분에, 특정 엔진만 교체·재학습·재인덱싱하는 변경이 다른 부분에 미치는 영향을 상대적으로 제한할 수 있습니다.
